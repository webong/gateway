...
(
    ...
    WEB_RELAY_HTTP_REGISTRY_SMOKE=1 \
    WEB_RELAY_PLANNER='Webong\WebRelay\Tests\Fixtures\HttpSmokePlanner' \
    WEB_RELAY_PATH_RESOLVER='Webong\WebRelay\Tests\Fixtures\HttpSmokePathResolver' \
...
benchmark_requests="${WEB_RELAY_HTTP_BENCHMARK_REQUESTS:-0}"
if [[ "$benchmark_requests" =~ ^[1-9][0-9]*$ ]]; then
    benchmark_concurrency="${WEB_RELAY_HTTP_BENCHMARK_CONCURRENCY:-20}"
    (
        cd "$ROOT"

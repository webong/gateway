...
APP_PATH="${NET_GATEWAY_HTTP_APP_PATH:-$ROOT/tests/Fixtures/octane-app}"
PHP_PORT="${NET_GATEWAY_HTTP_PHP_PORT:-18081}"
RELAY_PORT="${NET_GATEWAY_HTTP_RELAY_PORT:-18082}"
TOKEN="${NET_GATEWAY_INTERNAL_TOKEN:-http-smoke-token}"
REGISTRY_TOKEN="${REGISTRY_TOKEN:-registry-smoke-token}"
LOG_LEVEL="${NET_GATEWAY_HTTP_LOG_LEVEL:-error}"
PHP_URL="http://127.0.0.1:$PHP_PORT"
RELAY_URL="http://127.0.0.1:$RELAY_PORT"

TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/net-gateway-http.XXXXXX")"
...
(
    ...
    NET_GATEWAY_HTTP_RECEIVER_URL="http://127.0.0.1:$PHP_PORT/http-smoke/receiver" \
    NET_GATEWAY_HTTP_RECEIVER_FILE="$RECEIVER_FILE" \
    NET_GATEWAY_HTTP_REGISTRY_SMOKE=1 \
    NET_GATEWAY_PLANNER='Webong\NetGateway\Tests\Fixtures\HttpSmokePlanner' \
    NET_GATEWAY_PATH_RESOLVER='Webong\NetGateway\Tests\Fixtures\HttpSmokePathResolver' \
...
php_internal_status="$(curl -sS --max-time 5 -o /dev/null -w '%{http_code}' \
    -X POST "$PHP_URL/_internal/net-gateway/plan")"
...
go build -tags=netgateway_smoke "$GO_BUILD_FLAGS" -o "$RELAY_BIN" "$ROOT/cmd/relayer"

NET_GATEWAY_RUNTIME=http \
NET_GATEWAY_LARAVEL_BACKEND_URL="$PHP_URL" \
PORT="$RELAY_PORT" \
NET_GATEWAY_INTERNAL_TOKEN="$TOKEN" \
LOG_LEVEL="$LOG_LEVEL" \
...
internal_status="$(curl -sS --max-time 5 -o /dev/null -w '%{http_code}' \
    -X POST "$RELAY_URL/_internal/net-gateway/plan")"
...
benchmark_requests="${NET_GATEWAY_HTTP_BENCHMARK_REQUESTS:-0}"
if [[ "$benchmark_requests" =~ ^[1-9][0-9]*$ ]]; then
    benchmark_concurrency="${NET_GATEWAY_HTTP_BENCHMARK_CONCURRENCY:-20}"
    (
        cd "$ROOT"
